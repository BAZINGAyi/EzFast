"""
Raw asynchronous database implementation using native async/await.

This module provides true asynchronous database operations using SQLAlchemy's
async engine and asyncio drivers (asyncpg, aiomysql, aiosqlite).
It's suitable for high-concurrency scenarios requiring non-blocking I/O.
"""
...
from .db_base import DatabaseBase, DatabaseConnectionError

# import logging
# logging.basicConfig()
# logging.getLogger("sqlalchemy.engine").setLevel(logging.INFO)

class RawAsyncDB(DatabaseBase):
    """
    True asynchronous database implementation using async engine and drivers.

    This class provides native asynchronous database operations without thread pool wrapping.
    Requires async database drivers like asyncpg, aiomysql, or aiosqlite.

    Note: Uses async engine with native async/await for non-blocking operations.
    ...
    def _create_engine(self) -> None:
        """
        Create SQLAlchemy async engine based on configuration.
    ...
    async def get_session(self):
        ...
        This async context manager automatically handles:
    ...
    async def get_raw_session(self):
        """
        Get session for fastapi request Dependency Injection.
        """
        async with self._session_factory() as session:
            try:
                yield session
                ...
                await session.rollback()   # 出错时自动回滚
                raise
            finally:
                await session.close()      # 自动关闭连接
    ...
    async def make_table(self, table: Any, metadata: Optional[MetaData] = None) -> Table:
        """
        创建或获取 SQLAlchemy 表对象（支持异步 autoload）。

        :param table: 表名（str）或者 Table 对象，或者 ORM 模型
        ...
        if isinstance(table, str):
            ...
            # 如果缓存中已有，直接返回
            if table_name in self._table_definitions_cache:
                return self._table_definitions_cache[table_name]
    ...
    async def execute_query_stmt(self, stmt, return_clear=False):
        """
        Execute a SQL statement asynchronously.

        Args:
            stmt: The SQL statement to execute.
            return_clear: Whether to return clear dict results.

        Returns:
            Query results as list of dicts or fetchall result.
        """
    ...
    async def run_query(
        ...
        Args:
            table: 表对象或表名字符串
            select_columns: 选择的列，可以传入列名字符串列表
            where_conditions: WHERE 条件（字典形式）
            group_by_columns: GROUP BY 列，可以传列名字符串列表
            order_by_columns: ORDER BY 列，可以传列名字符串列表
            limit: 限制返回的结果数量
            offset: 偏移量
            return_clear: 是否返回清晰的结果（字典形式）

        Returns:
            查询结果列表
        """
        # 如果传入的是字符串，创建Table对象
        table = await self.make_table(table)
    ...
    async def _prepare_bulk_operation(self, table, operation_type: str, statistics_key: Optional[str] = None):
        """
        准备批量操作的通用初始化。
    ...
    async def bulk_insert_data(self, table, data: list, statistics_key=None):
        ...
        Args:
            table: SQLAlchemy 表对象或表名字符串
            data: 要插入的数据列表，每个元素为字典
            statistics_key: 统计信息的键名，如果不提供则使用表名_insert

        Returns:
            (status, err_msg, statistics) 元组
                - status: 操作是否成功
                - err_msg: 错误信息列表
                - statistics: 统计信息字典
        """
        # 使用公共方法初始化
        table, statistics = await self._prepare_bulk_operation(table, "insert", statistics_key)
    ...
    async def bulk_update_data(self, table, data: list, where_key: str = "id", statistics_key: Optional[str] = None):
        """
        ...
        Note:
            - 每个记录必须包含where_key字段
            - 适用于大数据量的简单更新
            - 如需复杂WHERE条件，请使用bulk_dml_table方法
        """
        table, statistics = await self._prepare_bulk_operation(table, "update", statistics_key)
        total_count = len(data)
    ...
    async def bulk_dml_table(self, table_data: list, open_transaction: bool = True) -> tuple:
        """
        ...
        try:
            async with self.get_conn() as conn:
                ...
                for operation_data in table_data:
                    ...
                    operation_data['table'] = await self.make_table(table)

                # 根据open_transaction参数决定是否使用事务
                if open_transaction:
                    async with conn.begin():
                        await self._execute_bulk_operations(conn, table_data, statistics_list, error_messages)
                else:
                    # 不使用事务，每个操作独立执行
                    await self._execute_bulk_operations(conn, table_data, statistics_list, error_messages)
    ...
    async def _execute_bulk_operations(self, conn, table_data: list, statistics_list: list, error_messages: list):
        """
        执行批量操作的核心逻辑。
    ...
    async def bulk_dml_table_sql(self, sql_statements: list, open_transaction: bool = True) -> tuple:
        """
        批量执行原生SQL语句（增删改操作）。
    ...
    async def add(self, model_class, data: Dict[str, Any]) -> tuple:
        """
        使用 ORM 方式添加单条记录到表中。
    ...
    async def update(self, model_class, data: Dict[str, Any], filter_condition) -> tuple:
        """
        ...
        try:
            async with self.get_session() as session:
                ...
                if affected_rows == 1:
                    # 查询更新后的记录
                    result_obj = await session.execute(select(model_class).where(filter_condition))
                    updated_record = result_obj.scalars().first()
                    if updated_record:
                        return True, updated_record.to_dict()
    ...
    async def delete(self, model_class, filter_condition) -> tuple:
        """
        ...
        try:
            async with self.get_session() as session:
                delete_stmt = delete(model_class).where(filter_condition)
