...
This module provides synchronous database operations built on top of the base database class.
It's suitable for traditional web applications and scripts that don't require high concurrency.
"""
...
from sqlalchemy import text, Result, MetaData, Table, delete, select, func
from sqlalchemy.sql.expression import text
from sqlalchemy.exc import SQLAlchemyError

from .db_base import DatabaseBase, DatabaseConnectionError

class SyncDB(DatabaseBase):
    """
    Synchronous database implementation using engine-based operations.
    
    This class provides synchronous database operations with run_query functionality
    similar to the AsyncDB implementation.
    ...
    def scroll_query(
        ...
        """
        # 先查询总记录数
        total_count_results = self.run_query(
            table=table,
    ...
    def bulk_update_data(self, table, data: list, where_key: str = "id", statistics_key: Optional[str] = None):
        ...
        Example:
            ...
            ... ]
            >>> db.bulk_update_data("users", update_data, where_key="id")
    ...
    def bulk_dml_table(self, table_data: list, open_transaction: bool = True) -> tuple:
        ...
        Example:
            ...
            ... ]
            >>> # 使用事务（默认）
            >>> success, errors, stats = db.bulk_dml_table(table_operations)
            >>> # 不使用事务
            >>> success, errors, stats = db.bulk_dml_table(table_operations, open_transaction=False)
        """
        # 验证输入参数
    ...
    def bulk_dml_table_sql(self, sql_statements: list, open_transaction: bool = True) -> tuple:
        ...
        Example:
            ...
            ... ]
            >>> # 使用事务执行
            >>> success, errors, stats = db.bulk_dml_table_sql(sql_statements)
            >>> # 不使用事务
            >>> success, errors, stats = db.bulk_dml_table_sql(sql_statements, open_transaction=False)
    ...
    def drop_tables(self, metadata) -> None:
        """
        Drop database tables from metadata.
        ...
        try:
            ...
            self.logger.error(f"Table dropping failed: {str(e)}")
            raise
    ...
    def get_table_info(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved table info for: {table_name}")
            return columns
        except SQLAlchemyError as e:
    ...
    def get_table_names(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(tables)} table names")
            return tables
        except SQLAlchemyError as e:
